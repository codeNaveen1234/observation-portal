  ...
  navigateToProjectPlayer(project: any) {
    ...
    this.apiService.get(urlConfig.observation.project + `${project?._id}`)
      ...
      .subscribe((res: any) => {
        ...
        if (result?.projectId) {
          window.location.href = `/managed-learn/project-details?type=details&id=${result?.projectId}&_id=${result?.projectId}&solutionId=${result?.solutionId}`;
        } else {
          window.location.href = `/managed-learn/project-details?externalId=${project?.externalId}&referenceFrom=observation`;
        }
      });
  }
}
